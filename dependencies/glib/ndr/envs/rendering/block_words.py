...
def draw_robot(ax, robot_width, robot_height, midx, midy, holding, block_width, block_height):
    ...
    if holding is not None:
        draw_name_on_rect(rect, ax, holding)
...
def draw_blocks(ax, block_width, block_height, block_positions):
    for block_name, (x, y) in block_positions.items():
        ...
        ax.add_patch(rect)
        draw_name_on_rect(rect, ax, block_name)

def draw_name_on_rect(rect, ax, block_name):
    rx, ry = rect.get_xy()
    cx = rx + rect.get_width()/2.0
    cy = ry + rect.get_height()/2.0
    ax.annotate(block_name.name.capitalize(), (cx, cy), color='w', 
