...
public class Wildfire2MDPGen {
	...
	public static void main(String [] args) throws Exception {
		...
		Wildfire2MDPGen gen = new Wildfire2MDPGen(args);
	...
	public  Wildfire2MDPGen(String [] args){
		output_dir = args[0];
...
//		}
		...
		sb.append("\tdomain = wildfire_2_mdp;\n");
		sb.append("\tobjects {\n");
				...
				for (int io = -1; io <= 1; io++)
					for (int jo = -1; jo <= 1; jo++) {
						...
						if (ran.nextFloat() < prob_drop_neighbor)
							sb.append("\t\t// Omitted: NEIGHBOR(l" + i + j + ",l" + xn + yn + ");\n");
						else
		...
		for (int i = 1; i <= size_x; i++)
			for (int j = 1; j <= size_y; j++)
				if (ran.nextFloat() < burn_prob) {
					sb.append("\t\tburning(l" + i + j + ");\n");
					any_burning = true;
				}
		if (!any_burning) // Must be at least one burning
			sb.append("\t\tburning(l11);\n");
		sb.append("\t};\n\n");
