...
public class CrossingTrafficMDPGen {
	...
	public boolean doesPathExist(boolean obs[][], int x, int y, int g_x, int g_y, 
		...
		if (x == g_x && y == g_y) {
		...
		else if (x < 0 || x >= obs.length) {
			
			return false;
		}
		else if (y < 0 || y >= obs[0].length) {
			
			return false;
		}
		else if (obs[x][y] == true) {
			...
			if (visited.contains(location)) {
				...
				return doesPathExist(obs, x, y + 1, g_x, g_y, visited)
						|| doesPathExist(obs, x, y - 1, g_x, g_y, visited)
						|| doesPathExist(obs, x, y + 1, g_x, g_y, visited)
						|| doesPathExist(obs, x + 1, y, g_x, g_y, visited)
						|| doesPathExist(obs, x - 1, y, g_x, g_y, visited);
			}
		}
	}
...
//        robot-at(x1,y1);
		...
		boolean obs[][] = new boolean[size_x][size_y];
		...
		for (int i = 1; i <= size_x; i++)
			for (int j = 2; j < size_y; j++)
				if (ran.nextBoolean()) {
					...
					if (this.doesPathExist(obs, size_x - 1, 0, size_x - 1, size_y - 1,
							new HashSet<String>())) {
