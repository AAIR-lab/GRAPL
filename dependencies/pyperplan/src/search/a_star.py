        ...
        if state_cost[pop_state] == pop_node.g:
            ...
            for op, succ_state in task.get_successor_states(pop_state):
                ...
                if not nnplact:
                    ...
                    if h == float("inf"):
                        # don't bother with states that can't reach the goal
                        # anyway
                        continue
                ...
                if succ_node.g < old_succ_g:
                    ...
                    if nnplact:
                        ...
                        candidate_list.append(succ_node)
                    else:
                        heapq.heappush(open, make_open_entry(
                            succ_node, h, node_tiebreaker))

            if nnplact and len(candidate_list) > 0:

                heuristic.update_candidates(pop_node, candidate_list)
                for candidate in candidate_list:

                    node_tiebreaker += 1
                    heapq.heappush(open, make_open_entry(
                        candidate, candidate.h, node_tiebreaker))
