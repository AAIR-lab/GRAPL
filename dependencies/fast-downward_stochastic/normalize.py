...
class AxiomConditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules):
        ...
        app_rule_body = list(condition_to_rule_body(
            axiom.parameters, self.condition))
...
def remove_universal_quantifiers_with_axioms(task):
    def recurse(condition):
        ...
        if isinstance(condition, pddl.UniversalCondition):
            ...
            if not axiom:
                typed_parameters = [pddl.TypedObject(
                    v, type_map[v]) for v in parameters]
...
def remove_universal_quantifiers_without_axioms(task):
    def lit_replacement(condition, obj_map):
        if isinstance(condition, pddl.Literal):
            ...
            new_parts = [lit_replacement(part, obj_map)
    ...
    def recurse(condition):
        ...
        if isinstance(condition, pddl.UniversalCondition):
            ...
            for combo in object_combos:
                copies.append(lit_replacement(template, dict(
                    zip(types, [obj.name for obj in combo]))))
...
def move_existential_quantifiers(task):
    def recurse(condition):
        ...
        if isinstance(condition, pddl.ExistentialCondition):
            new_parameters = condition.parameters + \
                existential_parts[0].parameters
