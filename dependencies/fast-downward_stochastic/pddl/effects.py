...
def parse_effects(alist):
    ...
    for (cost_eff, rest_effect) in cost_rest_eff:
        ...
        if cost_eff:
            ...
            # Here we can use the probability to assign a cost to help
            # heuristics
...
def parse_effect(alist):
    ...
    if tag == "and":
        ...
        assert 1 == len(
            effects), "Error: Cannot embed non-determinism inside of a forall (for now)."
        ...
        # Generate effects for each outcome and then set their probability
        # appropriately
        assert (len(
            alist) - 1) % 2 == 0, "Each probabilistic outcome must have an associated probability"
        ...
        for pair in outcome_pairs:
            ...
            for eff in effects:
                # Apply the base probability in the pair to this individual
                # effect
    ...
    def instantiate(self, var_mapping, init_facts, fluent_facts,
        ...
        if self.parameters:
            ...
            for object_tuple in cartesian_product(*object_lists):
                ...
                self._instantiate(var_mapping, init_facts,
                                  fluent_facts, result)
        else:
...
class ConditionalEffect(object):
    def __init__(self, condition, effect, probability=1):
        if isinstance(effect, ConditionalEffect):
            ...
            probability = Fraction(
                probability).limit_denominator() * effect.probability
        else:
            self.condition = condition
            self.effect = effect
    ...
    def dump(self, indent="  "):
        if (self.probability < 1):
            print("%s%.2f%% chance of" %
                  (indent, float(self.probability) * 100))
            indent += "  "
        print("%sif" % (indent))
        self.condition.dump(indent + "  ")
        print("%sthen" % (indent))
    ...
    def normalize(self):
        ...
        if isinstance(norm_effect, ConjunctiveEffect):
            ...
            cond_effect = ConditionalEffect(
                self.condition, child, self.probability)
    ...
    def normalize(self):
        ...
        if isinstance(norm_effect, ConjunctiveEffect):
            ...
            for effect in norm_effect.effects:
                ...
                new_effects.append(UniversalEffect(
                    self.parameters, effect, self.probability))
