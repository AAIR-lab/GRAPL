...
/* not yet preprocessed plan facts
 */
PlNode *gorig_plan_facts = NULL;
...

{
  ...
  if (plan_file ==0) {
    ...
    sprintf(pln_file, "%s%s", gcmd_line.path, gcmd_line.plan_file_name);
  }
...
if (plan_file == 0) {
  ...
  /*** Modified/Added by Pulkit***/
  if (plan_file == 1) {
    found_plan = apply_plan();
  }
  times( &end );
  TIME( gsearch_time );

  if (plan_file == 0 && found_plan ) {
    print_plan();
  } else if (plan_file == 1 && found_plan) {
    printf("\n\nFinal State:");
