...
/*** Added by Pulkit ***/
/* This does not actually make actions, it just encodes the 
   actions parsed into a structure Action, then generates
   states after each action is executed.
*/
void make_Actions(PlNode *n)
{
  int m, i;
  TokenList *t;
  ...
  while (node) {
	...
	k++;
        if(strcmp(node->atom->item,goperators[i]->name) == 0) {
          if ( ( a->norm_operator &&
	  ...
	  TokenList *token = node->atom;
          for ( j = 0; j < goperators[i]->number_of_real_params; j++ ) {
	    token = token->next;
	    if (strcmp (gconstants[a->name_inst_table[j]], token->item) != 0) {
	      break;
	    }
          }
	  if(j == goperators[i]->number_of_real_params) {
              /*Match Found*/
              gplan_ops[gnum_plan_ops++]=k;
	  }
	  /*Else No match*/
	}
      }
