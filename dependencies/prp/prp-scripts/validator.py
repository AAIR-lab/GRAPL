...
def validate(dfile, pfile, sol, val):
    ...
    # Create a state based graph for Stochastic AIA.
    # This graph does not need to be a multi-graph and the only difference
    # is that nodes are labeled with states here.
    G_state_based = nx.DiGraph()
    
    G.add_node(1, label="I")
    G_state_based.add_node(1, label=_state_string(unfluents, init_state))
    
    G.add_node(2, label="G")
    G_state_based.add_node(2, label="")
    ...
    while open_list:
        ...
        if not a:
            ...
            # Deadends do not require a label.
            G_state_based.nodes[nodes[u]]['label'] = _state_string(unfluents,
                                                                   u)
            unhandled.append(u)
        else:
            i = 0
            temp_a = copy.deepcopy(a)
            temp_a = temp_a.split("_")
            if temp_a[1].startswith("v"):
                t = temp_a[1].lstrip("v")
                if t.isnumeric():
                    temp_a.remove(temp_a[1])
            a = "_".join(temp_a)
            for outcome in actions[a]:
                ...
                if v.is_goal(goal_fluents):
                    G_state_based.nodes[2]["label"] = _state_string(unfluents, 
                                                                    v)
                    v = goal_state
                elif v not in nodes:
                    nodes[v] = node_index
                    ...
                    G_state_based.add_node(nodes[v], 
                                           label=_state_string(unfluents, v))
                    open_list.append(v)

                G.add_edge(nodes[u], nodes[v], label="%s (%d)" % (a, i))
                G_state_based.add_edge(nodes[u], nodes[v], label="%s" % (a))
    ...
    write_dot(G_state_based, 'graph_state_based.dot')
...
if __name__ == '__main__':
    ...
    # Added for debugging
    # dom = "/home/local/ASUAD/pverma13/Dropbox/Code/stochastic_aia/saia/src/../results/explodingblocks/q6/domain.pddl"
    # prob = "/home/local/ASUAD/pverma13/Dropbox/Code/stochastic_aia/saia/src/../results/explodingblocks/q6/problem.pddl"
    # sol = "/home/local/ASUAD/pverma13/Dropbox/Code/stochastic_aia/saia/src/../results/explodingblocks/q6/human_policy.out"
    # interp = "prp"
    #
    # validate(dom, prob, sol, importlib.import_module("validators.%s" % interp))
