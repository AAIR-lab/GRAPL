...
# Implicitly add equality preconditions when printing pddl strings.
ENFORCE_UNIQUE_PARAMS_FOR_PRECONS = True
...
class Operator:
    ...
    def determinize(self, with_copy=True):
        ...
        assert operator.is_optimized
        
        assert isinstance(operator.effects, ProbabilisticEffect)
        operator.effects = operator.effects.determinize(with_copy=False)
    ...
    def flatten(self, with_copy=True):
        ...
        if isinstance(operator.effects, Literal):
            ...
            literals = [operator.effects.flatten(with_copy=False)]
            operator.effects = ProbabilisticEffect([LiteralConjunction(literals)],
                                                    [1.0])
            operator.effects = operator.effects.flatten(with_copy=False)
        elif isinstance(operator.effects, LiteralConjunction):
            
            probabilistic_effect = None
            isolated_literals = []
            for literal in operator.effects.literals:
                
                if isinstance(literal, Literal):
                    
                    isolated_literals.append(literal.flatten(with_copy=False))
                elif isinstance(literal, LiteralConjunction):
                    
                    isolated_literals.extend(literal.flatten(with_copy=False))
                elif isinstance(literal, ProbabilisticEffect):
                    
                    assert probabilistic_effect is None
                    probabilistic_effect = literal.flatten(with_copy=False)
                else:
                    assert False
                    
            if probabilistic_effect is None:
                
                operator.effects = ProbabilisticEffect(
                    [LiteralConjunction(isolated_literals)], [1.0])
                operator.effects = operator.effects.flatten(with_copy=False)
            else:
                
                for effect in probabilistic_effect.literals:
                
                    effect.extend(copy.deepcopy(isolated_literals))
                    
                operator.effects = probabilistic_effect
                operator.effects = operator.effects.flatten(with_copy=False)
        else:
    ...
    def optimize(self, with_copy=True):
        ...
        assert operator.is_flattened

        assert isinstance(operator.preconds, LiteralConjunction)

        operator.effects = operator.effects.optimize(with_copy=False)
        assert isinstance(operator.effects, ProbabilisticEffect)
        
        operator.is_optimized = True
    ...
    def __init__(self, name, params, preconds, effects):
        ...
        self.action_predicate = None
        self.enforce_unique_params = ENFORCE_UNIQUE_PARAMS_FOR_PRECONS
    ...
    def ground(self, params, with_copy=True):
        ...
        params_map = {operator.params[i]: params[i] 
                      for i in range(len(operator.params))}
        
        # The params always get deep-copied no matter what.
        operator.params = copy.deepcopy(params)
        operator.preconds = operator.preconds.ground(params_map, 
                                                     with_copy=False)
        operator.effects = operator.effects.ground(params_map, 
                                                   with_copy=False)

        return operator

    def get_pddl_precon_begin_str_for_non_equality(self, params):

        
        if self.enforce_unique_params and len(self.params) > 1:
            
            string = "(and "
            for param_combo in itertools.combinations(params, 2):
                
                string += "(not (= %s %s)) "  % (param_combo[0].name,
                                                 param_combo[1].name)
        else:
    ...
    def get_pddl_precon_end_str_for_non_equality(self, params):
    ...
    def pddl_str(self):
        ...
        s+= self.get_pddl_precon_begin_str_for_non_equality(self.params)
        
        to_scan = [self.preconds]
        if isinstance(self.preconds, LiteralDisjunction):
            to_scan = copy.deepcopy(self.preconds.literals)
            s += "(or "
        for _i in to_scan:
            preconds_pddl_str = self._create_preconds_pddl_str(_i)
            s += "\n\t\t\t(and {})".format(preconds_pddl_str)
        if isinstance(self.preconds, LiteralDisjunction):
            s += ")"
            
        s += self.get_pddl_precon_end_str_for_non_equality(self.params)
        # s += ")"
...
class PDDLDomain:
    ...
    def initialize_non_fluents(self):

        only_negated_fluents = self.predicates.keys() - self.actions
        fluent_names = set()
        for operator in self.operators.values():

            operator = operator.flatten(with_copy=True)
            operator = operator.optimize(with_copy=False)
            assert isinstance(operator.effects, ProbabilisticEffect)

            effects = operator.effects.literals \
                      + [operator.effects.common_effects]

            for effect in effects:
                assert isinstance(effect, LiteralConjunction)
                for literal in effect.literals:

                    if not literal.is_anti:
                        only_negated_fluents.discard(literal.predicate.name)

                    fluent_names.add(literal.predicate.name)

        self.non_fluents = self.predicates.keys() \
                           - self.actions \
                           - fluent_names
        self.only_negated_fluents = only_negated_fluents - self.non_fluents
        pass

    def get_non_action_predicates(self):

        return [self.predicates[name]
                for name in self.predicates
                if name not in self.actions]

    def initialize_action_predicates_from_operators(self):
...
def parse_ff_plan_step(plan_step, domain, problem):
    
    return parse_plan_step(plan_step, domain.operators.values(),
                           domain.actions, problem.objects)
