...
class SimulatorOutOfBudgetException(Exception):

    pass

def get_successor_state(state, action, domain, 
                        raise_error_on_invalid_action=False, 
                        inference_mode="infer", require_unique_assignment=True,
                        get_all_transitions=False, return_probs=False,
                        grounded_actions_in_state=False, 
    ...
    selected_operator, assignment = _select_operator(state, action, domain, 
        ...
        grounded_actions_in_state=grounded_actions_in_state, 
        all_grounded_actions=all_grounded_actions)
    ...
    if assignment is not None:
        ...
        if isinstance(selected_operator.effects, LiteralConjunction):
            ...
            assert isinstance(selected_operator.effects,
                              (Literal, ProbabilisticEffect))
    ...
    # No assignment is found, action is not really applicable.
    if return_probs:
        assert len(state) > 0 and (get_all_transitions or len(state) == 1)
        state = [{state: 1.0}]
    else:
        state = [state]
...
def get_successor_states(state, action, domain, 
                         raise_error_on_invalid_action=False,
    ...
    return get_successor_state(state, action, domain, 
                               raise_error_on_invalid_action, inference_mode,
...
def _select_operator(state, action, domain, inference_mode="infer",
    ...
    if grounded_actions_in_state:
        kb = set(state.literals) - all_grounded_actions | {action}
    else:
...
class PDDLEnv(gym.Env):
    ...
    def __init__(self, domain_file, problem_dir, render=None, seed=0,
        ...
        self.grounded_actions_in_state = grounded_actions_in_state
        ...
        self._problem_initial_state_copy = None
    ...
    def reset_total_steps(self, value=0):
    ...
    def get_applicable_actions(self):
        ...
        return self._action_space.all_ground_literals(self._state, 
                                                      valid_only=True)

    def get_all_grounded_actions(self):

        return self.all_grounded_actions

    def get_actions(self, state=None, applicable_only=False):
        ...
        if not applicable_only:
            ...
            for action in self.all_grounded_actions:
                ...
                if self.is_action_applicable(action, state=state):
                    actions.append(action)

            return actions

    def is_action_applicable(self, action, state=None):

        if state is None:
            state = self.get_state()

        return self._action_valid_test(state, action)
    ...
    def reset(self):
        ...
        if self._problem:
            ...
            self._problem.initial_state = self._problem_initial_state_copy
        ...
        self._problem_initial_state_copy = self._problem.initial_state
        self._problem.initial_state = frozenset(initial_state.literals)
    ...
    def step(self, action, checked=False):
        ...
        if self.total_steps >= self.max_steps:
            raise SimulatorOutOfBudgetException()

        assert not self.shared_instance or checked
        ...
        if self.step_callback is not None:
            ...
            self.step_callback(self.total_steps)
    ...
    def sample_transition(self, action):
        state = self._get_successor_state(
            self._state, action, self.domain, 
            inference_mode=self._inference_mode,
            raise_error_on_invalid_action=self._raise_error_on_invalid_action,
            grounded_actions_in_state=self.grounded_actions_in_state,
            all_grounded_actions=self.all_grounded_actions)
