...
class FStripsParser(fstripsVisitor):
    ...
    def _configure_error_handling(self, element):
        if self.error_handler is not None:
            # If necessary, _replace_ previous error handlers with the given
            # one
    ...
    def visitEitherTypename(self, ctx):
        raise UnsupportedLanguageFeature(
            '"either"-based types not supported in Tarski PDDL parser')
    ...
    def visitSingle_predicate_definition(self, ctx):
        ...
        argument_types = [a.sort for a in self.visit(
            ctx.possibly_typed_variable_list())]
        return self.language.predicate(predicate, *argument_types)

    def visitUntypedVariableList(self, ctx):
        variables = [self.language.variable(
            name.getText().lower(), 'object') for name in ctx.VARIABLE()]
        return variables

    def visitTypedVariableList(self, ctx):
        untyped_var_names = [self.language.variable(
            name.getText().lower(), 'object') for name in ctx.VARIABLE()]
        typed_var_names = []
    ...
    def visitVariable_list_with_type(self, ctx):
        # This is the type of all variables in the list
        typename = self.visit(ctx.typename())
    ...
    def visitUntyped_function_definition(self, ctx):
        # According to Daniel Kovacs' PDDL 3.1 BNF spec, the default type for
        # functions is 'number'
    ...
    def visitTypeBoundsDefinition(self, ctx):
        ...
        if not isinstance(sort, Interval):
            raise ParsingError(
                "Attempt at bounding symbolic non-interval sort '{}'".format(sort))
    ...
    # For a fixed problem, there's no particular distinction btw domain
    # constants and problem objects.
    ...
    def visitActionDef(self, ctx):
        ...
        # Never parse action definitions.
        # For RL, we only need the predicates and goals.
        return
        ...
        if len(cost_effects) > 1:
            raise SyntaxError(
                f'Ill-formed action "{name}" with multiple cost effects: {cost_effects}')
        self.problem.action(name, binding, precondition,
                            effects, cost_effects[0] if cost_effects else None)
    ...
    def _recover_variable_from_context(self, name):
        if self.current_binding is None:
            raise ParsingError(
                "Variable '{}' used declared outside variable binding".format(name))
    ...
    def visitGenericFunctionTerm(self, ctx):
        ...
        subterms = tuple(s.to_constant(x) if not isinstance(
            x, Term) else x for s, x in zip(fun.domain, subterms))
        return fun(*subterms)
    ...
    def visitConjunctiveEffectFormula(self, ctx):
        ...
        for sub_ctx in ctx.single_effect():
            ...
            # MultipleCOnditionalEffects might return a list instead of a
            # single effect
    ...
    def visitInitNegativeLiteral(self, ctx):
        # No need to do anything here, as atoms are assumed by default to be
        # false
        pass
    ...
    def visitInitFunctionAssignment(self, ctx):
        ...
        subterms = tuple(s.to_constant(x)
                         for s, x in zip(fun.domain, subterms))
        self.init.set(fun(*subterms), value)

    def visitFlat_atom(self, ctx):
        predicate = self.language.get_predicate(
            ctx.predicate().getText().lower())
        subterms = tuple(self.visit(term_ctx)
                         for term_ctx in ctx.constant_name())
        return predicate, subterms

    def visitFlat_term(self, ctx):
        func_name = self.language.get_function(
            ctx.function_name().getText().lower())
        subterms = tuple(self.visit(term_ctx)
                         for term_ctx in ctx.constant_name())
    ...
    def visitProblemMetric(self, ctx):
        opt_type = OptimizationType.from_string(
            ctx.optimization().getText().lower())
    ...
    def visitTotalTimeMetric(self, ctx):
        raise UnsupportedLanguageFeature(
            "Unsupported feature: Minimize total-time metric is not supported")

    def visitIsViolatedMetric(self, ctx):
        raise UnsupportedLanguageFeature(
            "Unsupported feature: Count of violated constraints metric is not supported")

    def visitAssignEffect(self, ctx):
        operation = ctx.assignOp().getText().lower()
        lhs = self.visit(ctx.functionTerm())
        rhs = self.visit(ctx.term())
        operator = {'scale-up': '*', 'scale-down': '/',
                    'increase': '+', 'decrease': '-'}[operation]
        rhs = self.language.dispatch_operator(
            get_function_from_symbol(operator), Term, Term, lhs, rhs)
        return FunctionalEffect(lhs, rhs)

    def visitDerivedDef(self, ctx):
        raise UnsupportedLanguageFeature(
            "Parsing of derived predicates in Tarski not yet implemented")
...
class ParserVariableContext:
    ...
    def __enter__(self):
        ...
        if self.root and self.parser.current_binding is not None:
            raise ParsingError(
                "Clean ParserVariableContext opened upon existing context")
