...
(:predicates (clear ?v0 - blocks) (on-table ?v0 - blocks) (arm-empty) (holding ?v0 - blocks) (on ?v0 - blocks ?v1 - blocks))
	...
	(:action unstack
		...
		:precondition 
			...
			(not (clear ?underob))
			(not (arm-empty)))
		:effect (and  (not (clear ?ob)) (not (on ?ob ?underob)) (not (on ?underob ?ob)) (arm-empty) (clear ?underob) (holding ?ob) (holding ?underob) (probabilistic 1.000000 (and
			)))
	)
)