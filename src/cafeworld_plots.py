...
def get_title_for_domain(domain):
    ...
    elif domain == "Cafeworld":

        return "Cafe Server Robot"
    else:
...
def plot_supplemental(results_dir, domains, domain_data, num_bins):
    ...
    gs = gridspec.GridSpec(ncols=1, nrows=1, figure=fig, wspace=0.04)
    ...
    fig.legend(
        labels=["QACE (Ours)", "_b"],
        loc="center", ncols=4,
        bbox_to_anchor=(0.66, 0.82), frameon=False, fontsize=LEGEND_FONTSIZE)

    fig.text(0.19, -0.06, "Learning Time (minutes)", fontsize=XLABEL_FONTSIZE)

    fig.savefig("%s/cafe_world.pdf" % (results_dir), bbox_inches="tight")
    fig.savefig("%s/cafe_world.png" % (results_dir), bbox_inches="tight")
