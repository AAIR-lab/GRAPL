...
def do_stuff(ax):
    ...
    # transform_ticklabels(ax)
    # transform_ticklabels(ax, axis="y")

    if domain["source"] == "first_responders":
        ax.set_xlabel("Simulator Steps", fontsize=16)

    if domain["source"] == "blocks":
        pass
    ...
    # for task_no in range(TOTAL_TASKS):
    #     with plt.rc_context({'path.sketch': (1, 20, 1)}):
    #         ax.axvline(task_no * STEPS_PER_TASK,
    #                           linestyle="solid",
    #                           color="grey",
    #                           alpha=0.3)
    #
    ...
    #         ax.text((task_no * STEPS_PER_TASK) + 3000, 0.95, "$M_%s, \delta_%s$" % (task_no, task_no),
    #                        verticalalignment="center",
    #                        transform=blended_transform,
    #                        fontsize=12)
...
if __name__ == "__main__":
    ...
    ALGS = [
        ...
        {"name": "drift",
         "color": "blue",
         "linestyle": "solid",
         "label": "Unknown Drift + Need-based (CLaP (Ours))"
         },

        {"name": "qace",
         "color": "red",
         "linestyle": "solid",
         "label": "Unknown Drift + Full"
         },

        {"name": "qace-stateless",
         "color": "green",
         "linestyle": "dashdot",
         "label": "Known Drift + Full"
         },
    ...
    # fig.text(0.47, 0.280, "Simulator Steps $|\Delta|$", fontsize=16)
    fig.text(0.095, 0.36, "Avg. Reward", rotation=90, fontsize=16)
    # fig.text(0.498, 0.05, "Tasks", fontsize=16)

    # https://matplotlib.org/stable/gallery/misc/fig_x.html
    # fig.add_artist(lines.Line2D([0.106, 0.106], [0.34, 0.64], color="black"))
    ...
    for column, domain in enumerate(DOMAINS):
        ...
        markers_on = []
        for i in range(0, (TOTAL_TASKS * STEPS_PER_TASK) // 100, 500):
            markers_on.append(i)
        ...
        #     blended_transform = transforms.blended_transform_factory(
        #         domain_ax.transData, domain_ax.transAxes)
        #     domain_ax.text((task_no * STEPS_PER_TASK) + 3000, 0.92, "$M_%s, \delta_%s$" % (task_no, task_no),
        #                    verticalalignment="center",
        #                    transform=blended_transform,
        #                    fontsize=12)
        #
        ...
        for alg in ALGS:
            ...
            alg_ax = fig.add_subplot(gs[0 + alg_no, column])

            if alg_no == 0:
                alg_ax.set_title(domain["name"], fontsize=18, pad=20)
            ...
            lh = alg_ax.plot(o_x, o_y, label=ORACLE["label"],
                      color=ORACLE["color"],
                      marker="x",
                      markevery=markers_on,
                      markersize=8,
                      linestyle=ORACLE["linestyle"])
            ORACLE["handle"] = lh
            ...
            alg_x, alg_y, alg_yerr = get_x_y(data, domain["source"], alg["name"],
                                 "Avg. Reward")
            lh = alg_ax.plot(alg_x, alg_y, label=alg["label"],
                      color=alg["color"],
                      linestyle=alg["linestyle"])
            alg["handle"] = lh
            ...
            if alg_no == 1:
                for task_no in range(TOTAL_TASKS):
                    ...
                    blended_transform = transforms.blended_transform_factory(
                        alg_ax.transData, alg_ax.transAxes)
                    alg_ax.text((task_no * STEPS_PER_TASK) + 3000, 1.15, "$M_%s, \delta_%s$" % (task_no, task_no),
                                verticalalignment="center",
                                transform=blended_transform,
                                fontsize=12)
    ...
    legend_order = ["oracle", "qlearning", "drift", "qace", "qace-stateless"]
    legend_labels = []
    legend_handles = []
    handler_map = {}
    for legend in legend_order:
    ...
    fig.legend(handles=legend_handles,
               ...
               handlelength=4,
               loc="center",
                bbox_to_anchor=(0.50, 1.05), frameon=False, fontsize=12)
