...
if __name__ == "__main__":
    ...
    fig = plt.figure(constrained_layout=False, figsize=(8, 2))
    # fig.text(0.47, 0.280, "Simulator Steps $|\Delta|$", fontsize=16)
    # fig.text(0.095, 0.39, "(b) Avg. Reward", rotation=90, fontsize=16)
    ...
    alg_ax = fig.add_subplot(gs[0, 0])
    alg_ax.set_xlabel("Simulator Steps", fontsize=16)
    alg_ax.set_ylabel("Avg. Reward", fontsize=16)
    ...
    for task_no in range(TOTAL_TASKS):
        with plt.rc_context({'path.sketch': (1, 20, 1)}):
            ...
            alg_ax.text((task_no * STEPS_PER_TASK), 1.07, "Task$_%s, \delta_%s$" % (task_no, task_no),
                        verticalalignment="center",
                        transform=blended_transform,
                        fontsize=12)

    o_x, o_y, o_yerr = get_x_y(data, DOMAINS[1]["source"], "oracle",
                               "Avg. Reward")
    ...
    alg_x, alg_y, alg_yerr = get_x_y(data, DOMAINS[1]["source"], ALGS[1]["name"],
                                     "Avg. Reward")
    alg_ax.plot(alg_x, alg_y, label=ALGS[1]["label"],
                color=ALGS[1]["color"],
                linestyle=ALGS[1]["linestyle"])

    transform_ticklabels(alg_ax)

    fig.legend(
        labels=["_", "_", "_", "_", "_", "Oracle", "Model-based RL"],
        ncols=2,
        loc="center",
        bbox_to_anchor=(0.53, 1.05), frameon=False, fontsize=14)
    ...
    for column, DOMAIN in enumerate(DOMAINS):
        ...
        for run_no in RUNS:
            ...
            for metric in metrics:
                ...
                data["qace-stateless"][X_METRIC][run_no], data["qace-stateless"][metric][run_no] = get_data(
                    qace_stateless_file,
                    X_METRIC, metric, alg, metric)

                data["drift"][X_METRIC][run_no], data["drift"][metric][run_no] = get_data(drift_file,
                                                                                          X_METRIC, metric, "drift",
                                                                                          metric)
